...
%     lambda_i = paramVn; %paramVn
    ...
    [s_res, ~, p_res, ~,converged] = SPOQ_BD_quadraFidel(y_woBaseline, L, p,q,theta,B, iter_o, gamma, iter_max,...
...
%     save(['param/param_BD_testp_p_',num2str(p), '_q_', num2str(q),'_',dataId,'_lambda.mat'],'paramVn')
    save(['param/param_backcorSPOQ_simplex2_p_',num2str(p), '_q_', num2str(q),'_',dataId,'.mat'],'paramVn')
else
    disp([dataId,' not ok'])
    paramVn=Simplex('centroid'); % obtain the final value if ok
%     save(['param/param_BD_testp_p_',num2str(p), '_q_', num2str(q),'_',dataId,'_lambda.mat'],'paramVn')
